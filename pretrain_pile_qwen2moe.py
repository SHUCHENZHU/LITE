...
# from modelling_qwen2_moe.configuration_qwen2_moe import Qwen2MoeConfig

# from modelling_qwen2_moe.modelling_qwen2_moe import Qwen2MoeForCausalLM 
...
def main(args):
    ...
    elif args.model_config=='qwen2-2B':  
        Num_experts_per_token=4
        Num_experts=32
        model_config = Qwen2MoeConfig(
            vocab_size=32000,
...
#         model = Qwen2MoeForCausalLM(model_config)      
    ...
    def compile_moe_parts(model):

        for i, layer in enumerate(model.model.layers):

            layer.mlp = torch.compile(layer.mlp )

            layer.input_layernorm = torch.compile(layer.input_layernorm )
    ...
    if args.use_torch_compile and hasattr(torch, 'compile') and torch.cuda.is_available():
        try:
            logger.info(f"[{time.time():.2f}] Compiling model with torch.compile for optimization")
            # mode="reduce-overhead" is suitable for training scenarios
            model = compile_moe_parts(model)
            logger.info(f"[{time.time():.2f}] Model (mlp,norm) compilation completed")
    ...
    if args.optimizer.lower() == "muonlite":
        ...
        muon_params = [
            ...
            if p.ndim >= 2 and "embed_tokens" not in name and "lm_head" not in name and "shared_expert_gate" not in name 
        ]
        adamw_params = [
            (name, p) 
            for name, p in model.named_parameters()
            if not (
                p.ndim >= 2 and "embed_tokens" not in name and "lm_head" not in name and "shared_expert_gate" not in name 
            )
        ]
...
# -----------------------------------------------------------------------------------------
        ...
        z_loss = torch.tensor(0.0, device=loss.device)
        ...
        if outputs.router_logits:
            ...
            for layer_logits in outputs.router_logits:
                # z-loss 
                z_loss += torch.logsumexp(layer_logits, dim=-1).pow(2).mean()
            
            z_loss = z_loss / len(outputs.router_logits)
        
        total_loss = loss + (router_z_loss_coef * z_loss)
        
        scaled_loss = total_loss /  grad_accumulation
        scaled_loss.backward()
        
        current_aux_loss = (outputs.aux_loss).detach().item()
        current_z_loss = z_loss.detach().item() 
        ...
        if update_step % args.eval_every == 0:
            current_eval=True
        if update_step >= int(args.stable_ratio*args.num_training_steps) and update_step % 200==0:
            current_eval=True
            ...
            swanlab.log(
                {
                    ...
                    "train/router_z_loss": current_z_loss,
